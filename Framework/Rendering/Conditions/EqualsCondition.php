...
class EqualsCondition implements ConditionInterface
{
    ...
    public function __construct(?string $var1 = null, ?string $var2 = null, bool $isLiteral = false)
    {
        $this->var1 = $var1;
        $this->var2 = $var2;
    ...
    public static function fromExpression(string $expression): ConditionInterface
    {
        if (!preg_match('/^([\w\.]+)\s*==\s*([\'"]?)([\w\.]+)\2$/', $expression, $matches)) {
            throw new \InvalidArgumentException("Expresión no válida para EqualsCondition: $expression");
        }
